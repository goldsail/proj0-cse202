...
export class MapContainer extends React.Component {
  ...
  render() {
    return <div>
      ...
      <div>
        {this.state.routes
        ? <p>Optimal route: {this.state.routes.map(p => <span>{p}</span>).reduce((prev, curr) => [prev, ' -> ', curr])}</p>
        : <p>Please input your preference score for each place and click "Calculate"</p>}
      </div>
      <Map
        style={{
          width: '100%',
